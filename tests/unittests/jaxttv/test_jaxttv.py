...
def get_ttvfast_times(jttv, pdic, nplanet, obs=True, **kwargs):
    ...
    _, tcs_ttvfast = get_ttvfast_model(pdic_ttvfast.iloc[0], nplanet, jttv.t_start, jttv.dt, jttv.t_end, **kwargs)
...
def get_ttvfast_times_and_rvs(jttv, pdic, nplanet, times_rv, obs=True, **kwargs):
    ...
    pdic_ttvfast = params_for_ttvfast(p, jttv.t_start, nplanet)
    _, tcs_ttvfast, rvs = get_ttvfast_model_rv(pdic_ttvfast.iloc[0], nplanet, jttv.t_start, jttv.dt, jttv.t_end,times_rv,  **kwargs)
    tcs_ttvfast = np.hstack(tcs_ttvfast)
...
def test_get_transit_times_obs_nontransiting():
    ...
    pdic = elements_to_pdic(*params_to_elements(params_, 4), force_coplanar=False)
    ...
    chi2_ref = np.sum( ((tc_tf - jttv.tcobs_flatten) / jttv.errorobs_flatten)**2 )
    assert np.isclose(chi2, chi2_ref, rtol=1e-2)

    # insufficient precision for this parameter set?
    pdic = elements_to_pdic(*params_to_elements(params2_, 4), force_coplanar=False)
    pdic['pmass'] = pdic['mass']
...
def test_get_transit_times_all_nontransiting():
    ...
    tc_jttv, _, oidx = jttv.get_transit_times_all(pdic, transit_orbit_idx=transit_orbit_idx)
    tcs_ttvfast = get_ttvfast_times(jttv, pdic, jttv.nplanet+1, obs=False, skip_planet_idx=[3])
    
    #tc1, tc2 = jttv.check_timing_precision(pdic)
    #t_prec = np.max(np.abs(tc1 - tc2))
    #print (t_prec) # 1.4e-5
...
def test_get_transit_times_and_rvs_obs():
    ...
    tc_ttvfast, rv_ttvfast = get_ttvfast_times_and_rvs(jttv, pdic, jttv.nplanet, times_rv)
